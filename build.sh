...
sudo docker build -t davask/d-apache-letsencrypt-proxy-reverse:${branch} ${thisDir};

echo "sudo docker run --name d-apache-letsencrypt-proxy-reverse -tdi -p 80:80/tcp -p 2222:22/tcp -p 443:443/tcp --label io.rancher.scheduler.affinity:host_label=dwl=dwlComPrivate davask/d-apache-letsencrypt-proxy-reverse:${branch}";
